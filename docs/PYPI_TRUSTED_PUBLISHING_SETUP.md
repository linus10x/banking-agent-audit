...
- Maintainer with `Owner` role on the `linus10x/banking-agent-audit` GitHub repo
...
2. Under **Add a new pending publisher**, fill in:
   - PyPI Project Name: `banking-agent-audit`
   - Owner: `linus10x`
   - Repository name: `banking-agent-audit`
...
The first successful publish creates `https://pypi.org/project/banking-agent-audit/` and pins the binding.
...
1. Open https://github.com/linus10x/banking-agent-audit/settings/environments
...
2. The `build` job produces `dist/banking_agent_audit-1.2.0-py3-none-any.whl` and `dist/banking_agent_audit-1.2.0.tar.gz`.
...
4. Verify https://test.pypi.org/project/banking-agent-audit/1.2.0/ renders, the version shows a `Sigstore signed` badge, and the **Download files** view shows two artifacts plus an attestation bundle.
5. Smoke-install in a scratch venv:
   ...
   pip install -i https://test.pypi.org/simple/ \
       ...
       banking-agent-audit==1.2.0
   python -c "import banking_agent_audit; print(banking_agent_audit.__version__)"
   ```
...
1. Confirm `pyproject.toml`, `src/banking_agent_audit/__init__.py`, and `CITATION.cff` all read `1.2.0` and the `date-released` is set to the real ship date.
2. Tag and push:
...
5. Verify https://pypi.org/project/banking-agent-audit/1.2.0/ — `Sigstore signed` badge present, both artifacts visible, attestation bundle attached.
...
```bash
pip install --require-hashes banking-agent-audit==1.2.0
pip inspect banking-agent-audit | grep -i provenance
```

The `provenance` field in `pip inspect` output (or `pip install --verbose` console output) will reference the Sigstore bundle and the `linus10x/banking-agent-audit` workflow + commit SHA that produced the build.
